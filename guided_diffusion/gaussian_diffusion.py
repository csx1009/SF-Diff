...
from uilt.utils import calculate_3D_HaarPSI, calculate_3D_HaarPSI_DEBUG
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            if self.model_mean_type == ModelMeanType.START_X:
                ...
                pred_xstart_unclamped = self._predict_xstart_from_eps(x_t=x_t, t=t, eps=model_output)
                pred_xstart = process_xstart(
                    pred_xstart_unclamped)
        ...
        if self.mode == 'default':
            ...
            assert (model_mean.shape == pred_xstart.shape) and (model_log_variance.shape == x.shape)
        else:
    ...
    def ddim_sample_loop_progressive(
        ...
        for i, i_prev in zip(indices, indices_prev):  # 1000 -> 0
            if segmentation_img is not None:
                prev_img = img
                img = th.cat((img, segmentation_img), dim=1)
            t = th.tensor([i] * shape[0], device=device)
    ...
    def training_losses(self, model, x_start, t, classifier=None, model_kwargs=None, noise=None, labels=None,
        ...
        if mode == 'default':
            ...
            x_t = self.q_sample(labels, t, noise=noise)
            x_t = th.cat([x_t, x_start], dim=1)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            # mse = mean_flat((labels - model_output) ** 2)
            # labels_mean = labels.mean(dim=list(range(1, len(labels.shape))), keepdim=True)
            # variance = mean_flat((labels - labels_mean) ** 2)
            # epsilon = 1e-8
            # terms["mse"] = mse / (variance + epsilon)
            terms["mse"] = mean_flat((labels - model_output) ** 2)

            # print("mas_loss:",mas_loss)
            # terms["mse"] = mean_flat((labels - model_output) ** 2)
            # print("calculate_3D_HaarPSI calculate_3D_HaarPSI calculate_3D_HaarPSI calculate_3D_HaarPSI")
            # loss_list = [
            #     1 - calculate_3D_HaarPSI_DEBUG(labels[:, i, ...].unsqueeze(1), model_output[:, i, ...].unsqueeze(1))
            #     for i in range(labels.shape[1])]
            # # for i in range(labels.shape[0]):
            # loss_tensor = torch.stack(loss_list)
            #
            # terms["HaarPSI_loss"] = loss_tensor.mean(dim=list(range(1, len(loss_tensor.shape)))).unsqueeze(0)

            # terms["mse"] = torch.tensor(loss_list).to(labels.device)

            # if terms["mse"] > 10 or not torch.isfinite(terms["mse"]):
            #     print("ATTENTION MSE loss greater than 10")
            # if "vb" in terms:
            #     raise ValueError('we should not be using vb')
            #     terms["loss"] = terms["mse"] + terms["vb"]

            # terms["loss"] = 0.5 * terms["mse"] + 0.5 * terms["HaarPSI_loss"]
            terms["loss"] = terms["mse"]
            # terms["mse"] = terms["loss"]
        else:
